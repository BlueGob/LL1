		...
		if(checkForRecursion(iterator)){
			...
			for (int i = 0; i < iterator->count; ++i)
			{
				if(iterator->production[i][0] != iterator->name[0]){
					...
					strcat(temp->production[k],temp->name);
					strcat(temp->production[k],"'");
					++k;
				}else{
					char* help = substring(iterator->production[i],1,strlen(iterator->production[i])-1);
...
char * removeEpsilon(char * premier){
	...
	char * temp = malloc(strlen(premier)-l);
	l=0;
	for (int i = 0; i < strlen(premier); ++i)
	{
		if(premier[i] != '&'){
...
void main(){
	...
	switch (verify){
		case 1:
			...
			break;
		case 0:
			printf("the word %s is not accepted  \n",text);
			break;
	}
